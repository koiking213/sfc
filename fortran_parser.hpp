...
namespace client {
  ...
  struct fortran_parser : qi::grammar<Iterator, spirit::utree::list_type(), qi::locals<std::string> > {
    ...
    qi::rule<Iterator, std::string()> Arrow;
    qi::rule<Iterator, spirit::utree::list_type()> PROGRAM, END, USE;
  public:
	...
	//-( -(',' >> modul_enature) >> qi::skip(qi::ascii:blank)DOUBLESEMI)
	qi::omit[blank] >> name >> -(qi::skip(qi::ascii::blank)[','] >> qi::omit[*qi::ascii::blank] >> rename_list);

      // todo
      rename_list = rename % ',';
      rename = name >> qi::skip(qi::ascii::blank)[Arrow] >> qi::skip(qi::ascii::blank)[name];
